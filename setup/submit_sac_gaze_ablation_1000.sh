...
array_job_id="$(
    sbatch --parsable \
        ...
        setup/sac_gaze_ablation_1000.sbatch
)"
aggregate_job_id="$(
    sbatch --parsable \
        --dependency="afterok:${array_job_id}" \
        --export="ALL,QUEST_GIT_COMMIT=${git_commit},EXP05_ARRAY_JOB_ID=${array_job_id}" \
        setup/sac_gaze_ablation_aggregate.sbatch
)"
printf '%s|%s|%s\n' "${array_job_id}" "${aggregate_job_id}" "${git_commit}"
REMOTE
} 2>&1)" || {
    echo "Quest EXP-05 submission failed:" >&2
    echo "${result}" >&2
    exit 1
}

IFS='|' read -r array_job_id aggregate_job_id git_commit <<<"${result}"
echo "Submitted EXP-05 array: ${array_job_id}"
echo "Submitted EXP-05 aggregate: ${aggregate_job_id} (afterok:${array_job_id})"
